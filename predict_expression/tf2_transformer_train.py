...
from models.official.transformer import compute_bleu
from models.official.transformer.utils import tokenizer
from models.official.transformer.v2 import data_pipeline
from models.official.transformer.v2 import metrics
from models.official.transformer.v2 import misc
from models.official.transformer.v2 import optimizer
from models.official.transformer.v2 import transformer
from models.official.transformer.v2 import translate
from models.official.utils.flags import core as flags_core
from models.official.utils.logs import logger
from models.official.utils.misc import keras_utils
from models.official.utils.misc import distribution_utils
...
global X_train, y_train, geneName_train, X_valid, y_valid, geneName_valid, X_test, y_test, geneName_test, params

def main():
    #python deep_motif_learning_hyperopt.py train 2rand6tpe2anneal.hyperopt spearmint_data
    ...
    if not options.usemodel:
        trainfile = h5py.File(os.path.join(datadir, 'train.h5'), 'r') #_mouse1to1
        X_train, y_train, geneName_train = trainfile[''], trainfile['label'], trainfile['geneName']
        validfile = h5py.File(os.path.join(datadir, 'valid.h5'), 'r') #_mouse1to1
        X_valid, y_valid, geneName_valid = validfile[''], validfile['label'], validfile['geneName']

    testfile = h5py.File(os.path.join(datadir, 'test.h5'), 'r')
    X_test, y_test, geneName_test = testfile[''], testfile['label'], testfile['geneName']
    ...
    if not params['usemodel']:
        ...
        X_trainSubseq = X_train[:,leftpos:rightpos,:]
        X_validSubseq = X_valid[:,leftpos:rightpos,:]
        input = Input(shape=X_trainSubseq.shape[1:], name='')
    ...
    if params['tuneMode']:
        result = model.fit([X_trainSubseq, X_trainhalflife], y_train, batch_size=int(params['batchsize']), shuffle="batch", epochs=10,
                            validation_data=[X_validSubseq, y_valid])
        ...
        X_testSubseq = X_test[:,leftpos:rightpos,:]
        if not params['usemodel']:
            check_cb = ModelCheckpoint(os.path.join(params['datadir'], 'trainepoch.{epoch:02d}-{val_loss:.4f}.h5'), monitor='val_loss', verbose=1, save_best_only=True, mode='min')
            earlystop_cb = EarlyStopping(monitor='val_loss', patience=5, verbose=1, mode='min')
            result = model.fit(X_trainSubseq, y_train, batch_size=int(params['batchsize']), shuffle="batch", epochs=100,
                validation_data=X_validSubseq, y_valid], callbacks=[earlystop_cb, check_cb])
            mse_history = result.history['val_mean_squared_error']
            mse = min(mse_history)
            best_file = os.path.join(params['datadir'], 'trainepoch.%02d-%.4f.h5' % (mse_history.index(mse), mse))
            model = load_model(best_file)
            print 'Loaded results from:', best_file

        predictions_test = model.predict([X_testSubseq], batch_size=20).flatten()
        ...
        print "Best Validation MSE = %.3f" % mse
