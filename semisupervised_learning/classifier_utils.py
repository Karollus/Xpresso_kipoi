...
class yeastProcessor(DataProcessor):
  """Processor for DNA data sets."""
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      text_a = ' '.join(text_a)
      if set_type != "test":
        label = float(self.process_text(line[2]))
      else:
        label = 0
      examples.append(
...
class yeastORFProcessor(DataProcessor):
  """Processor for DNA data sets."""
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      guid = self.process_text(line[0])
      text_a = self.process_text(line[1])
      #if single nuc codon model, comment next 2 lines if codon model
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if task_name != "sts-b" and task_name != "yeast":
    label_map = {}
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["label_ids"] = create_float_feature([feature.label_id])\
        if task_name == "sts-b" or task_name == "yeast" else create_int_feature([feature.label_id])
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  labeltype = tf.float32 if task_name == "sts-b" or task_name == "yeast" else tf.int64
...
def model_fn_builder(albert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      if task_name not in ["sts-b", "cola", "yeast"]:
      ...
      elif task_name == "sts-b" or task_name == "yeast":
        def metric_fn(per_example_loss, label_ids, logits, is_real_example):
